...
def parse_data(train_path, test_path):
    ...
    test_y = []
    train_x.to_csv('{}/train_set.seg_x.txt'.format(BASE_DIR), index=None, header=False)
    train_y.to_csv('{}/train_set.seg_y.txt'.format(BASE_DIR), index=None, header=False)
    test_x.to_csv('{}/test_set.seg_x.txt'.format(BASE_DIR), index=None, header=False)
